window.onload = function(){
    ...
    $.getJSON("http://localhost:3000/api/nav",function(obj){
            ...
            <td class="goods"><img src="http://localhost:3000${item.image}" alt=""/><span>${item.name}</span></td>
            <td class="price">${item.price}</td>
            <td class="count"><span class="reduce"></span><input class="count-input" type="text" value="${arr[i].count}"/><span class="add">+</span></td>
            <td class="subtotal">${arr[i].count*getPrice(item.price)}元</td>
            <td class="operation"><span class="delete">删除</span></td>
                ...
                function getSubtotal(tr) {
                    ...
                    //写入HTML
                    console.log(parseFloat(getPrice(price.innerHTML)));
                    subtotal.innerHTML = (parseInt(countInput.value) * parseFloat(getPrice(price.innerHTML))).toFixed(2);
    ...
    $(".account").click(function(){
    	var arr = [];
    	$("tr.on").each(function(index,val){
    		var obj = {};
    		obj.id = $(this).attr("data-id");
    		obj.count = $(this).find(".count-input").val();
    		arr.push(obj);
    	});
